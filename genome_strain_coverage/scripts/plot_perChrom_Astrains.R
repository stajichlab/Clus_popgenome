...
for ( i in c(1:8) ) {
    ...
    pdffile = sprintf("plot/SC_%d_Astrains.pdf",i)
    Title=sprintf("SC %d - all strains",i)
    cov = read.table(filename,header=T,sep="\t")
    groups = cov$GROUP
    covA = subset(cov,cov$GROUP =="A")
    print(summary(covA))
#    print(covA)
    
    covA$GENENUM = as.numeric(gsub("CLUG_","",covA$GENE))
    covA$POSITION = as.numeric(covA$POSITION)
    #maxGene= max(covA$GENENUM)
    #print(maxGene)
    g = ggplot(covA,aes(POSITION,COVERAGE,color=STRAIN,shape=LOCALE,
                        group=LOCALE)) +
   ...
   for ( j in c("L", "S", "U") ) {
    ...
    p <- ggplot(covLoc,
   aes(x=POSITION,y=COVERAGE,color=STRAIN)) +
   labs(title=ctgTitle) + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
   scale_shape(solid=T) + geom_point(alpha=0.9,size=1.5) + scale_colour_brewer(palette = "Paired") 
   
   pdffile = sprintf("plot/SC_%d_Astrains_%s.pdf",i,j)
