...
for my $chrom (sort keys %chroms ) {
    ...
    print $ofh join("\t", qw(GENE POSITION), @strains_final), "\n";
    print $normofh join("\t", qw(GENE POSITION), @strains_final), "\n";
    for my $genear ( sort { $a->[0] <=> $b->[0] } @{$chroms{$chrom}} ) {
	my ($gstart,$gend, $gene) = @$genear;
	print $ofh join("\t", $gene, $gstart, map { $genecov{$gene}->{$_} } @strains_final), "\n";
	print $normofh join("\t", $gene, $gstart,
